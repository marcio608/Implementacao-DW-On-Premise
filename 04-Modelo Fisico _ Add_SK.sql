...
-- Fornece privilégios ao usuário dw na stage area
...
-- Criando a Surrogate Key(SK) que será usada como PK. Por exemplo, no caso da tabela TB_DIM_CLIENTE o campo ID_CLIENTE será uma Natural Key.
-- SK é uma coluna com valores incrementais.
-- Essa estratégia será usada para criar as tabelas TB_DIM_CLIENTE, TB_DIM_PRODUTO, TB_DIM_LOCALIDADE


CREATE SEQUENCE dim_cliente_id_seq START WITH 100 INCREMENT BY 1 NOCACHE NOCYCLE; -- gerando a SK
...
-- Tabela FATO
-- Cada fato será identificado de maneira única pela combinação cliente, produto, localidade e data.
...
(
  ...
  SK_DATA INTEGER NOT NULL, 
  VL_VENDA DECIMAL(12,4), --fato
  QTD_VENDA INTEGER, --fato
  DATA_CARGA DATE,
